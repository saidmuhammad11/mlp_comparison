...
This script:
    ...
    5. Saves SHAP summary, SHAP bar, and SHAP waterfall plots as SVG.
    6. Opens the SVG plots automatically using xdg-open.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no-open",
        action="store_true",
        help="Do not open saved SVG plots automatically.",
    )
    parser.add_argument(
        "--matplotlib-show",
        action="store_true",
    ...
    plt.rcParams.update(
        {
            ...
            "ytick.labelsize": 14,
            "legend.fontsize": 14,
            # Keep text as editable text in SVG output instead of converting it to paths.
            "svg.fonttype": "none",
        }
    )
...
def open_plots(paths: list[Path]) -> None:
    print("\nOpening saved SHAP SVG plots with xdg-open...")
    for path in paths:
        try:
            subprocess.Popen(
                ["xdg-open", str(path)],
            ...
            print("Could not find xdg-open. Open the SVG files manually from the output folder.")
            return
        except Exception as exc:
...
def main() -> None:
    ...
    summary_path = out_dir / f"shap_summary_beeswarm_class_{args.class_index}_{class_label.lower()}.svg"
    bar_path = out_dir / f"shap_mean_abs_importance_class_{args.class_index}_{class_label.lower()}.svg"
    waterfall_path = out_dir / f"shap_waterfall_sample_{args.sample_index}_class_{args.class_index}_{class_label.lower()}.svg"
    ...
    plot_paths = [summary_path, bar_path, waterfall_path]
