#!/usr/bin/env python3
"""
Journal-ready comparison plots and numerical summaries for HistGBDT-RPM.

This revision uses one common analysis pipeline for every reported value,
upgraded to support N-run averaging for Task-to-Completion metrics:

1. Each log is converted to elapsed time relative to its own first sample.
2. The maximum duration across all runs is used to build a common time grid.
3. All runs are interpolated onto the same grid, and numeric values are averaged.
4. Categorical states are flattened across all runs to calculate true distribution.
...
BASELINE_FILES = [
    './results/workload_mixed2_bursty_mg_medium_run1_20260923_102054/monitoring.jsonl',
    './results/workload_mixed2_bursty_mg_medium_run2_20260923_102654/monitoring.jsonl',
    './results/workload_mixed2_bursty_mg_medium_run3_20260923_103258/monitoring.jsonl',
    './results/workload_mixed2_bursty_mg_medium_run4_20260923_103857/monitoring.jsonl',
    './results/workload_mixed2_bursty_mg_medium_run5_20260923_104501/monitoring.jsonl',
]

PROPOSED_FILES = [
    './results/workload_mixed2_bursty_mg_hgbdt_run1_20260923_080704/monitoring.jsonl',
    './results/workload_mixed2_bursty_mg_hgbdt_run2_20260923_081306/monitoring.jsonl',
    './results/workload_mixed2_bursty_mg_hgbdt_run3_20260923_081907/monitoring.jsonl',
    './results/workload_mixed2_bursty_mg_hgbdt_run4_20260923_082509/monitoring.jsonl',
    './results/workload_mixed2_bursty_mg_hgbdt_run5_20260923_083108/monitoring.jsonl',
]
...
OUTPUT_PREFIX = "HistGBDT_RPM"

# This is dynamically calculated based on the longest run in your file lists.
...
# STRICT_DURATION_CHECK MUST BE FALSE for Task-To-Completion, as the entire point
# is that the proposed model finishes at a different time than the baseline.
STRICT_DURATION_CHECK = False
...
def load_json_log(filename: str) -> pd.DataFrame:
    ...
    path = Path(filename)
    if not path.exists():
        print(f"Input log not found, skipping: {path.resolve()}")
        return pd.DataFrame()
    ...
    if not records:
        print(f"No valid records were found in {filename}")
        return pd.DataFrame()
...
class JournalComparator:
    """Create all figures and tables from averaged multi-run datasets."""

    def __init__(
        self,
        baselines: list[pd.DataFrame],
        proposeds: list[pd.DataFrame],
        workload_name: str,
    ) -> None:
        self.baselines = baselines
        self.proposeds = proposeds
        self.workload_name = workload_name
        self.time_s = build_common_time_grid()
        self.time_min = self.time_s / 60.0
        self.metrics = available_metric_pairs(baselines[0], proposeds[0])
    ...
    def aligned_metric(self, column: str) -> tuple[np.ndarray, np.ndarray]:
        """Return cached baseline/proposed arrays averaged on the common grid."""
        if column not in self._aligned_cache:
            base_arrays = []
            for i, df in enumerate(self.baselines):
                base_arrays.append(
                    interpolate_numeric_metric(df, column, self.time_s, f"{BASELINE_LABEL} Run {i+1}")
                )
                
            prop_arrays = []
            for i, df in enumerate(self.proposeds):
                prop_arrays.append(
                    interpolate_numeric_metric(df, column, self.time_s, f"{PROPOSED_LABEL} Run {i+1}")
                )
                
            self._aligned_cache[column] = (
                np.mean(base_arrays, axis=0),
                np.mean(prop_arrays, axis=0)
            )
    ...
    def detect_workload_type(self) -> str:
        ...
        avg_l3_miss = (
            ...
            if "l3_miss_rate" in self.proposeds[0].columns
            and "l3_miss_rate" in self.baselines[0].columns
            else 0.0
        )
        avg_mem_bw = (
            self.metric_statistics("memory_bandwidth")["proposed_mean"]
            if "memory_bandwidth" in self.proposeds[0].columns
            and "memory_bandwidth" in self.baselines[0].columns
            else 0.0
        )
    ...
    def create_combined_absolute_delta_plot(self) -> None:
        ...
        metrics = [
            ...
            if column in self.baselines[0].columns and column in self.proposeds[0].columns
        ]
        if not metrics:
            raise ValueError(
    ...
    def create_box_plot(self) -> None:
        ...
        metrics = [
            ...
            if pair[0] in self.baselines[0].columns and pair[0] in self.proposeds[0].columns
        ]
        if not metrics:
            return
    ...
    def create_performance_level_plot(self) -> None:
        """Plot time share in each controller state concatenated across all runs."""
    ...
    def create_cache_plot(self) -> None:
        ...
        if not required.issubset(self.baselines[0].columns) or not required.issubset(
            self.proposeds[0].columns
        ):
            return
    ...
    def export_summary_csv(self) -> None:
        """Write aligned numerical results and Task-to-Completion metrics to CSV."""
        metric_specs = [
        ...
        for display_name, column, unit, scale in metric_specs:
            if column not in self.baselines[0].columns or column not in self.proposeds[0].columns:
                continue
        ...
        # ---------------------------------------------------------
        # TASK TO COMPLETION METRICS EXTRACTOR
        # ---------------------------------------------------------
        base_times = [float(b["elapsed_s"].iloc[-1]) for b in self.baselines]
        prop_times = [float(p["elapsed_s"].iloc[-1]) for p in self.proposeds]
        
        avg_base_time = float(np.mean(base_times))
        avg_prop_time = float(np.mean(prop_times))
        
        rows.append(
            {
                "Metric": "Task Execution Time",
                "Unit": "s",
                BASELINE_LABEL: avg_base_time,
                PROPOSED_LABEL: avg_prop_time,
                "Baseline-referenced difference": avg_base_time - avg_prop_time,
                "Difference type": "Absolute Difference",
                "Change vs. baseline (%)": (
                    100.0 * (avg_prop_time - avg_base_time) / avg_base_time 
                    if avg_base_time != 0 else float("nan")
                ),
            }
        )
        ...
        if "instructions_retired" in self.baselines[0].columns:
            ...
            rows.append(
                {
                    "Metric": "Total Instructions Retired",
                    "Unit": "count",
                    BASELINE_LABEL: avg_base_inst,
                    PROPOSED_LABEL: avg_prop_inst,
                    "Baseline-referenced difference": avg_base_inst - avg_prop_inst,
                    "Difference type": "Absolute Difference",
                    "Change vs. baseline (%)": (
                        100.0 * (avg_prop_inst - avg_base_inst) / avg_base_inst
                        if avg_base_inst != 0 else float("nan")
                    ),
                }
            )
        ...
        if "cpu_cycles" in self.baselines[0].columns:
            ...
            rows.append(
                {
                    "Metric": "Total CPU Cycles",
                    "Unit": "count",
                    BASELINE_LABEL: avg_base_cycles,
                    PROPOSED_LABEL: avg_prop_cycles,
                    "Baseline-referenced difference": avg_base_cycles - avg_prop_cycles,
                    "Difference type": "Absolute Difference",
                    "Change vs. baseline (%)": (
                        100.0 * (avg_prop_cycles - avg_base_cycles) / avg_base_cycles
                        if avg_base_cycles != 0 else float("nan")
                    ),
                }
            )
        ...
        if "cpu_power" in self.baselines[0].columns and "cpu_power" in self.proposeds[0].columns:
            ...
            rows.append(
                {
                    ...
                    "Unit": "J",
                    BASELINE_LABEL: avg_base_energy,
                    PROPOSED_LABEL: avg_prop_energy,
            ...
            base_edp = avg_base_energy * avg_base_time
            prop_edp = avg_prop_energy * avg_prop_time
            ...
            rows.append(
                {
                    ...
                    "Unit": "J*s",
                    BASELINE_LABEL: base_edp,
                    PROPOSED_LABEL: prop_edp,
    ...
    def export_analysis_audit(self) -> None:
        ...
        baseline_interval = estimate_sampling_interval_s(self.baselines[0])
        proposed_interval = estimate_sampling_interval_s(self.proposeds[0])

        audit = {
            "workload_name": self.workload_name,
            "baseline_files_processed": len(self.baselines),
            "proposed_files_processed": len(self.proposeds),
            ...
            "baseline_endpoint_tolerance_s": endpoint_tolerance_s(self.baselines[0]),
            "proposed_endpoint_tolerance_s": endpoint_tolerance_s(self.proposeds[0]),
            "numeric_alignment": "linear interpolation to common fixed grid, averaged across N runs",
            "categorical_alignment": "last observation carried forward, stacked across N runs",
            "mean_definition": "trapezoidal integral divided by fixed duration",
            "delta_definition": "baseline minus proposed on common grid",
            "energy_definition": "trapezoidal integral of raw CPU power, averaged across runs",
        }
    ...
    def print_consistency_report(self) -> None:
        ...
        print("/nAligned consistency report")
        print("=" * 78)
        for column, ylabel in COMBINED_METRICS:
            if column not in self.baselines[0].columns or column not in self.proposeds[0].columns:
                continue
...
def main() -> None:
    ...
    baselines = [load_json_log(f) for f in BASELINE_FILES]
    proposeds = [load_json_log(f) for f in PROPOSED_FILES]

    # Filter out empty/missing files gracefully
    baselines = [b for b in baselines if not b.empty]
    proposeds = [p for p in proposeds if not p.empty]

    if not baselines or not proposeds:
        print("Error: Missing valid baseline or proposed log files. Please check paths.")
        return

    # Calculate global max duration to accommodate Task-to-Completion
    max_b_dur = max(float(b["elapsed_s"].iloc[-1]) for b in baselines)
    max_p_dur = max(float(p["elapsed_s"].iloc[-1]) for p in proposeds)
    
    global ANALYSIS_DURATION_S
    ANALYSIS_DURATION_S = max(max_b_dur, max_p_dur)

    print(
        f"Loaded {len(baselines)} baseline runs and {len(proposeds)} proposed runs."
    )
    print(
        f"Auto-configured interpolation grid to {ANALYSIS_DURATION_S:.2f} s "
        "to accommodate the longest completion time."
    )
    ...
    print(
        f"Completed averaged multi-run exports for {WORKLOAD_NAME} "
