...
9. Optionally creates baseline-minus-proposed delta plots.
...
class JournalComparator:
    ...
    def create_comprehensive_plot(self) -> None:
        """
        Create a compact full-width multi-panel time-series figure.

        All panels share the time axis by column. Only the bottom row contains
        x-axis labels, and the legend is shown once for the complete figure.
        """
        if not self.metrics:
            raise ValueError("No common plot metrics were found in the two logs")

        metrics = self.metrics[:8]
        rows = int(np.ceil(len(metrics) / 2))

        # Eight panels are placed across the full two-column width.
        fig_height = max(5.25, rows * 1.24)
        ...
        # Reserve a dedicated band above the first row for the shared legend.
        fig.subplots_adjust(
            left=0.10,
            right=0.99,
            bottom=0.10,
        ...
        for index, (column, ylabel) in enumerate(metrics):
            ...
            if legend_handles is None:
                legend_handles = [line_baseline, line_proposed]
                legend_labels = [BASELINE_LABEL, PROPOSED_LABEL]

        for index in range(len(metrics), rows * 2):
            row, col = divmod(index, 2)
        ...
        for col in range(2):
            ...
            if visible_axes:
                visible_axes[-1].set_xlabel("Time (min)")
        ...
        preview_and_maybe_save(fig, "comprehensive")
        plt.close(fig)

    def create_delta_plot(self) -> None:
        """
        Plot baseline minus proposed values.

        Proposed samples are linearly interpolated onto the baseline time grid,
        allowing the difference to be displayed when timestamps differ.
        """
        if not self.metrics:
            return
        ...
        metrics = self.metrics[:8]
        rows = int(np.ceil(len(metrics) / 2))
        fig_height = max(4.0, rows * 1.55)

        fig, axes = plt.subplots(
            rows,
            2,
            figsize=(FULL_WIDTH_IN, max(5.0, fig_height)),
            sharex="col",
            squeeze=False,
            constrained_layout=False,
        )
        ...
        for index, (column, ylabel) in enumerate(metrics):
            ...
            baseline_values = self.baseline[column].to_numpy(dtype=float)
            proposed_values = self.proposed[column].to_numpy(dtype=float)
            interpolated_proposed = np.interp(
                self.baseline_time,
                self.proposed_time,
                proposed_values,
            )
            delta = baseline_values - interpolated_proposed

            ax.plot(self.baseline_time, delta)
            ax.axhline(0.0, linewidth=0.7)
            ax.set_ylabel(f"Δ {ylabel}", labelpad=2.0)
            ax.tick_params(axis="both", which="major", pad=1.5)
            ax.grid(True)
            add_panel_label(ax, f"({chr(ord('a') + index)})")
        ...
        preview_and_maybe_save(fig, "delta")
        plt.close(fig)
...
def main() -> None:
    ...
    comparator.create_comprehensive_plot()
    comparator.create_delta_plot()
